...
spec = do
  describe "Category" $ do
    ...
    it "Result isPrimCate (Primitive \"s\") is True" $ do
      isPrimCate (getCateFromString "s") `shouldBe` (True :: Bool)
    it "Result isDeriCate (getCateFromString \"(s/np)/(s/np)\") is True" $ do
      isDeriCate (getCateFromString "(s/np)/(s/np)") `shouldBe` (True :: Bool)
